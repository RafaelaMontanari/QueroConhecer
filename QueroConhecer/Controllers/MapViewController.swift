...
//  QueroConhecer
//
//  Created by Rafaela on 05/07/20.
...
class MapViewController: UIViewController {
    ...
    // MARK: - Properties
    var places: [Place]!
    var poi: [MKAnnotation] = [] //point of interest
    lazy var locationManager = CLLocationManager() //será instanciado apenas no momento em que será usado
    ...
    func configureLocationButton() {
        ...
        btUserLocation.frame.origin.x = 10.0
        btUserLocation.frame.origin.y = 10.0
    ...
    func showMessage(type: MapMessageType) {
        ...
        let message = type == .authorizationWarning ? "Para utilizar os recursos de localição, é necessário permitir o uso na tela de Ajustes" : "Rota não encontrada"
...
extension MapViewController: UISearchBarDelegate {
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        search.start { (response, error) in
            if error == nil {
                guard let responde = response else {
...
extension MapViewController: CLLocationManagerDelegate {
    ...
    /*  if let location = locations.last{
        print("Velocidade: ", location.speed_t)
        let region = MKCoordinateRegion.init(location.coordinate, 500, 500)
